		...
		if (!paused) {
			...
			if (previous_state != null) {
				...
				if (Qtable.get(prev_stateaction) == null) {
					...
					var oldQ = Qtable.get(prev_stateaction);
					var N_state_action = Ntable.get(prev_stateaction);
					//see figure 23.8 page 854
					var newQ = oldQ + alpha(N_state_action) * (previous_reward + GAMMA_DISCOUNT_FACTOR * (getMaxActionQValue(new_state)) - oldQ);
